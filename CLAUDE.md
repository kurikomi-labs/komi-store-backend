...
- `.env` lives only on the VPS at `/opt/github-store-backend/.env`, never in git. Under `APP_ENV=production` the app refuses to start without these: `DATABASE_URL`, `DATABASE_PASSWORD`, `MEILI_URL`, `MEILI_MASTER_KEY`, `GITHUB_OAUTH_CLIENT_ID`, `GITHUB_OAUTH_CLIENT_SECRET`, `OAUTH_SERVICE_TOKEN`, `DEVICE_ID_PEPPER`. Also relied on: `OAUTH_WEB_CALLBACK_URL` (default `https://github-store.org/auth/callback`), `OAUTH_SERVICE_ALLOWED_HOSTS` (no default — empty + production = S2S OAuth refuses every request as a configuration trip-wire), `GITHUB_TOKEN` (quiet-window fallback), `GH_TOKEN_TRENDING` / `GH_TOKEN_NEW_RELEASES` / `GH_TOKEN_MOST_POPULAR` / `GH_TOKEN_TOPICS` (rotation pool), `ADMIN_TOKEN`, `SENTRY_DSN`, `TOKEN_QUIET_START_UTC` / `TOKEN_QUIET_END_UTC` (default 1 / 4), `BADGE_USER_COUNT`, `DATABASE_POOL_SIZE` (default 20), `DISABLE_LIVE_GITHUB_PASSTHROUGH` and `DISABLE_BADGE_FETCH` kill switches. `.env.example` is the canonical list.
