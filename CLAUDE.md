...
| `GET /search?q=&platform=&sort=&order=&limit=&offset=` | Meilisearch-powered search. Auto-triggers GitHub passthrough if <5 results. `sort` ∈ {`relevance` (default), `stars`, `recent` / `releases` (alias, by latest stable release date), `updated` (by repo `updated_at_gh`)}; `order` ∈ {`asc`, `desc`} (default `desc`, ignored for relevance). `relevance` requires `q`; the others allow empty `q` for browse-mode listings. **Every non-relevance sort routes to Postgres FTS** (#711 — Meili's rankingRules put relevance rules before `sort`, silently ignoring it on small result sets); Meili serves `sort=relevance` only. Forge sources (`source=codeberg` etc.) reject explicit sorts with 400. Reads optional `X-GitHub-Token` header to run passthrough on the user's 5000/hr quota instead of the backend's fallback quota. Response carries `passthroughAttempted: Boolean` so clients can distinguish "index was warm but returned nothing" from "GitHub also has nothing". |
...
| `GET /feed?platform=&page=&limit=` | Anonymous daily-rotation discovery feed (`docs/backend/feed-endpoint.md`). Four candidate pools (trending / fresh releases / hidden gems / popular) interleaved by `FeedAssembler` with owner≥8 / topic≥4 diversity windows, deterministic shuffle seeded by UTC epochDay — same response for every caller all day, so the CDN absorbs traffic (`max-age=900, s-maxage=3600`). `platform` optional (absent = all-platform mix), `page` 1–25, `limit` 1–50. Response carries `rotation` (UTC date tag) that clients key their local cache on. **Never reads `X-GitHub-Token`** — anonymity + shared cache correctness both depend on it. Snapshot rebuilt every 3h (daily seed kept — fresh releases fold in without reshuffling); restart rebuilds on first request. |
